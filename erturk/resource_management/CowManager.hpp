#ifndef ERTURK_COW_MANAGER_H
#define ERTURK_COW_MANAGER_H
...
template <class T, T*(CopyAllocator)(const T*)>
class CowManager final
{
public:
    explicit CowManager(const T* resource_ptr) : resource_control_ptr_{new ResourceControl_{resource_ptr}} {}

    // Apply pointer shallow copy and increase reference count
    CowManager(const CowManager& other) : resource_control_ptr_{other.resource_control_ptr_}
    {
        resource_control_ptr_->reference_count_.fetch_add(1, std::memory_order_relaxed);
    }

    // Transfer control ownership
    CowManager(CowManager&& other) noexcept : resource_control_ptr_{other.resource_control_ptr_}
    {
        other.resource_control_ptr_ = nullptr;
    }

    // Apply pointer shallow copy and increase reference count
    CowManager& operator=(const CowManager& other)
    {
        if (this != &other)
        {
            release_control();
    ...
    // Transfer control ownership
    CowManager& operator=(CowManager&& other) noexcept
    {
        if (this != &other)
        {
            release_control();
...
private:
    void detach_control()
    {
        if (resource_control_ptr_->reference_count_.load(std::memory_order_acquire) > 1)
        {
            ...
            T* new_resource_ptr = CopyAllocator(old_resource_control->resource_ptr_);
    ...
    ResourceControl_* resource_control_ptr_;
};

}  // namespace erturk::resource_management

#endif  // ERTURK_COW_MANAGER_H
