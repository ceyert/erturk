...
enum class InstantiatePolicy : unsigned char
{
    Copy,
    Move
};

// Emplace T with copy/move constructor from an already emplaced T array, into address
...
inline constexpr T_Iterator emplace_type_buffers_copy(T_Iterator emplaced_src_begin, const T_Iterator emplaced_src_end,
                                                      ...
                                                      InstantiatePolicy instantiatePolicy = InstantiatePolicy::Copy)
{
    ...
    try
    {
        while (emplaced_src_begin != emplaced_src_end)
        {
            ...
            if (instantiatePolicy == InstantiatePolicy::Copy)
            {
            ...
            {
                ::new (static_cast<void*>(&*curr_type_buffer))
                    T{std::move(*emplaced_src_begin)};  // Emplace T with move constructor from placed T, into address
            }
...
// Emplace T with copy/move constructor into address
template <class T, class T_Iterator>
                                                 ...
                                                 InstantiatePolicy instantiatePolicy = InstantiatePolicy::Copy)
{
    ...
    try
    {
        while (curr_type_buffer != type_buffer_dest_end)
        {
            ...
            {
                ::new (static_cast<void*>(&*curr_type_buffer))
                    T{value};  // Emplace T with copy constructor into address
            }
            else
            {
                ::new (static_cast<void*>(&*curr_type_buffer))
                    T{std::move(value)};  // Emplace T with move constructor into address
            }
...
// Emplace T with any constructor into address
                                                 ...
                                                 const T_Iterator type_buffer_dest_end, Args&&... args)
{
    ...
    try
    {
        while (curr_type_buffer != type_buffer_dest_end)
        {
            ...
            // buffer(not instantiated)
            ::new (static_cast<void*>(&*curr_type_buffer))
                T{std::forward<Args>(args)...};  // Emplace T with any constructor into address
